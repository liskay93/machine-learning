...
아는 것에 붙여서 이해하기
  · **Ridge = 베이지안 prior.** 계수에 β ~ N(0, τ²) 을 얹고 MAP 를 구하면 정확히 ridge 가 된다.
    ...
    표본 57분기에 변수 20개(팩터 4 × lag 5)인 상황에서, prior 없이 OLS 를 돌리면 자료가 잡음까지 설명해 버린다.
...
만드는 순서 (반드시 지킨다)
  0. 설계행렬에 AR 항(y_{q−1})을 넣지 않는다. 공모자산과 베타를 나란히 놓기 위한 제약이다.
  1. OLS(동분기 팩터만) 를 먼저 돌려 베이스라인 대비 성능을 본다.
