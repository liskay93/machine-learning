...
용어 두 개
──────────
**표본 밖 (out-of-sample, OOS)** — 모델이 학습에 쓰지 않은 구간.
  이 저장소에서는 이렇게 만든다: 2006~2011 자료로 모델을 만들어 2012Q1 을 예측 →
  2012Q1 을 포함해 다시 만들어 2012Q2 예측 → ... 2026Q1 까지. 매 분기 새로 적합하므로
  어느 예측도 자기 미래를 본 적이 없다. 실전에서 매 분기 하게 될 절차와 같다.
  표본 **안** 성적은 변수를 늘리기만 해도 올라가므로 (극단적으로 변수 수 ≥ 표본 수 면 R²=1.00)
  모델의 값어치를 재는 자로 쓸 수 없다.

**벤치마크** — "가장 멍청한 방법". 여기서는 확장창 평균(과거 평균을 그대로 찍기)이다.
  팩터도 안 보고 아무것도 안 하는 예측이라, 이것을 못 이기면 모델을 만든 의미가 없다.
  벤치마크도 반드시 실시간이어야 한다 — 평균을 전체 표본으로 계산하면 미래를 본 벤치마크가 되어
  비교 자체가 무효다. 반드시 그 시점까지의 자료로만 계산한다.


무엇을 메인으로 볼 것인가 (결정, 2026-09-22)
──────────────────────────────────────────
둘 다 적는다. 역할이 다르다.
  · **rmse 가 보고용.** 단위가 있어 그대로 읽힌다 — "분기 수익률을 평균 2.2%p 틀린다".
  · **r2_oos 가 판정용.** "이 모델을 쓸 가치가 있나" 에 답하는 것은 이쪽뿐이다.

읽는 순서
  1. r2_oos > 0 인가? 아니면 거기서 끝. 과거 평균만도 못한 모델이다.
  2. 그렇다면 rmse 를 본다. 보고서에 들어갈 숫자다.
  3. rmse 와 mae 가 크게 벌어지는지 본다. 벌어지면 위기 분기 몇 개가 성적을 지배한다.

r2_oos 만 메인으로 두지 않는 이유: 분모(벤치마크)에 따라 값이 널뛴다. 같은 모델이
"평균 대비 0.14 / AR(1) 대비 −0.40" 으로 동시에 성립한다. rmse 에는 그런 함정이 없다.

둘은 이렇게 이어진다 (벤치마크가 같을 때):
    r2_oos = 1 − (rmse_모델 / rmse_벤치마크)²
    buyout 실측: 1 − (2.19 / 3.79)² = 0.665
"""
